          ...
          <div v-if="true || partner.liveData">
            <div class="bg-gray-800 text-gray-100 pt-5">
              <h1 class="text-4xl pl-6 py-2 font-extrabold">
                ...
                <div class="livenow inline-block ml-2">
                  <div></div>
                  <div></div>
                  <div></div>
                </div>
            ...
            <span class="mt-6 flex flex-row">
              ...
              <div class="pl-6 pb-4">
                <h3 class="text-3xl text-gray-200 font-bold leading-8">
                  {{ partner.contact ? partner.contact.name : "" }}
                </h3>
...
const public_token =
  "T5mFXB4glB8PPm2RE2CtP1UA2hrcdREZjQoXo/+FRaR0zuRNdN8ha5Aq0HpYCvnW";
const API_URL = "https://1src.tech/api/things/%id/state";
...
export default {
  ...
  methods: {
    async fetchThingUpdate({ thingId, mapping }) {
      let response = (
        await axios.get(API_URL.replace("%id", thingId), {
          headers: {
      ...
      Object.entries(response.data)
        .filter(([property]) => property in mapping)
    ...
    expand(partner) {
      ...
      this.expandedPartner =
        this.expandedPartner.length === 0 ? partner.name : "";
    },
